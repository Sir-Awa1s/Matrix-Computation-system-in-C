...
int determinant(int row_one,int column_one)
{
    ...
    {
         mult *= matrix_RR[c][c];
         if(fabs(matrix_RR[c][c]) > EPSILON)
                {
                    rank++;
                } else if(fabs(matrix_RR[c][c]) < EPSILON)
                {
                    continue;
         }
    }
    ...
    printf("Rank of matrix: %d\n",rank);
     ...
     if(fabs(mult) < EPSILON_FLOAT)
    {
        printf("Determinant is 0!, cannot define INVERSE.\nPress enter to exit.");
        while((c=getchar()) != '\n' && c != EOF);
        getchar();
        exit(0);
    }else if(fabs(mult) > EPSILON_FLOAT)
    {
         printf("\n");
...
int inverse(int row_one,int column_one)
{
    ...
    double identity[row_one][column_one];
            ...
            for(int c=0;c<row_one;c++)
            {
                for(int d=0;d<column_one;d++) // FIRST LOOP TO MAKE IDENTITY MATRIX //
                {
                    identity[c][d]=0;
                }
            }
            ...
            {
                identity[w][w] = 1.0;  // SECOND LOOP TO MAKE ALL DIAGONALS 1 //
            }
    ...
    if(row_one < 3 && column_one < 3)
    {
        ...
        } else if(fabs(det) > EPSILON)
        {
            ...
            double elemOne = detTwo*matrix[1][1];
            double elemTwo = detTwo*(-matrix[0][1]);
            double elemThree = detTwo*(-matrix[1][0]);
            double elemFour = detTwo*matrix[0][0];
            
            matrix[0][0] = elemOne;
            matrix[0][1] = elemTwo;
            matrix[1][0] = elemThree;
            matrix[1][1] = elemFour;
        
            printf("Inverse for 2x2 matrix: \n");
            for(int i=0;i<row_one;i++)
            {
                printf("\n");
                for(int j=0;j<column_one;j++)
    ...
    else if (row_one > 2 && column_one > 2)
        {
            double matdetcheck[row_one][column_one];
            for(int a=0;a<row_one;a++)
            {
                for(int b=0;b<column_one;b++)
                {
                matdetcheck[a][b]=matrix[a][b];
                }
            }

            for(int p=0;p<row_one;p++)
            {
            if(fabs(matdetcheck[p][p]) < EPSILON)
            {
                for(int a=p+1;a<row_one;a++)
                {
                    if(fabs(matdetcheck[a][p]) >= EPSILON)
                    {
                        for(int b=0;b<column_one;b++)
                        {
                            double temp = matdetcheck[p][b];
                            matdetcheck[p][b]=matdetcheck[a][b];
                            matdetcheck[a][b] = temp;
                        }
                        break;
                    }
                }
            ...
            double pivot = matdetcheck[p][p];
            /*printf("pivot %d: %.2f\n",p,pivot);*/ //Pivot-PRINT Checker//
            for(int r=p+1;r<row_one;r++)
            {
                double multiplier = matdetcheck[r][p]/pivot;
                /*printf("multiplier %d: %.2f\n",r,multiplier);*/ //Multiplier-PRINT checker//

                for(int q=p;q<column_one;q++)
                {
                    matdetcheck[r][q]=matdetcheck[r][q]-(multiplier*matdetcheck[p][q]);
                }
            }
            }
            ...
            for(int c=0;c<row_one;c++)
            {
                mult *= matdetcheck[c][c];
                if(fabs(matdetcheck[c][c]) > EPSILON)
                {
                    rank++;
                } else if(fabs(matdetcheck[c][c]) < EPSILON)
                {
                    continue;
                }
            }
